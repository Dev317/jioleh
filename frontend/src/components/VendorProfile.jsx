...
const EditBtnStyle =
  "bg-red-500 text-white font-bold p-2 rounded-xl outline-none w-40";
        ...
        <p>
          {vendor.description == null ? (
            <span className="text-gray-400">
              vendor has not added a description
            </span>
          ) : (
            vendor.description
          )}
      ...
      <div className="flex mt-3 justify-center">
        ...
        <p>
          {vendor.location == null ? (
            <span className="text-gray-400">
              vendor has not added a location
            </span>
          ) : (
            vendor.location
          )}
      ...
      <div className="flex p-3 justify-center">
        ...
        <p>
          {vendor.category == null ? (
            <span className="text-gray-400">
              vendor has not added a category
            </span>
          ) : (
            vendor.category
          )}
  ...
  return (
    <div className="m-5 p-5 rounded overflow-hidden shadow-lg">
      ...
      <p className="break-words mt-3">
        From the <span className="font-bold">{startDate}</span> onwards,{" "}
        {vendor.name} will be offering rewards to new customers and their
        referrers!
      </p>
      <p className="break-words mt-3">
        For every friend you refer, you will be awarded{" "}
        <span className="font-bold">{vendor.rewardAmount}</span> ETH!
      </p>
      <p className="break-words mt-3">
        In return, said referred friend will receive{" "}
        <span className="font-bold">{vendor.refereeReward}</span> upon their
        first visit to our establishment!{" "}
        <span className="text-gray-400">
          (Minimum spending required to be met to claim reward)
        </span>
      </p>
      <p className="break-words mt-3">
        This promotion will be available for{" "}
        <span className="font-bold">{vendor.duration} Days</span> from the start
        date of the campaign.
      </p>
      {vendor.dailyLimit == null ? (
        <></>
      ) : (
        <p className="italic text-sm break-words mt-3">
          Limited to the first{" "}
          <span className="font-bold">{vendor.dailyLimit}</span> customer(s) per
        ...
        <div className="relative flex flex-col mb-2">
          <div className="flex flex-col justify-center items-center">
            <img
              src={
                vendor?.bgimage
                  ? vendor?.bgimage && urlFor(vendor?.bgimage).url()
                  : defaultBg
              }
            ...
            <img
              ...
              src={
                vendor?.image
                  ? vendor?.image && urlFor(vendor?.image).url()
                  : defaultProfile
              }
              alt="vendor-pic"
            />
          ...
          <h1 className="font-bold text-2xl text-center mt-3">{vendor.name}</h1>
          <h1 className="text-xl text-center">@{vendor.username}</h1>
              ...
              {loggedInVendor?._id === vendor._id ? (
                <div className="flex gap-2 justify-center p-7">
                  ...
                  <Link to={`/vendor/vendor-campaign-detail`}>
                    <button type="button" className={EditBtnStyle}>
                      Edit Campaign
                    </button>
                  </Link>
                </div>
